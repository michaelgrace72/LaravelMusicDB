    ...
    <div class="flex items-center">
        ...
        <div x-data="{ dropdownOpen: false }"  class="relative">
            ...
            <div x-cloak x-show="dropdownOpen" class="absolute right-0 z-10 w-48 mt-2 overflow-hidden bg-white rounded-md shadow-xl">
                ...
                @role('admin')
                <a href="{{ route('admin.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">Admin</a>@endrole
                <form method="POST" action="{{ route('logout') }}" x-data>
                    @csrf
                    <a href="{{ route('logout') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white" @click.prevent="$root.submit();">{{ __('Log Out') }}</a>
                </form>
            </div>
        </div>
    </div>
